...
| v5.2 | 2026-06-20 | [중간] Sheet 3 히트맵 죽은 코드 제거: RETURN_QUANTILE_COLORS 루프(col.quantile 잘못된 참조 가능) 삭제, quantiles[month] 직접 비교만 유지. §22 MSS 연동 설계 신규 추가: 사이드바 rank_table 메뉴 항목(mss_app.py 변경 3곳), tab_rank_table.py 전체 구현(DB 직접 읽기, 3-탭 구성, Styler 색상 적용), MFRT_DB_PATH 환경변수, 모바일 Phase 1 제외 방침, 투자 워크플로 통합 시나리오 |
...
## 22. Money Sprout Sniffer (MSS) 연동 설계

### 22.1 연동 개요

Money Sprout Sniffer는 종목 스캔·관찰종목·섹터 트렌드·급등분석을 제공하는 Streamlit 기반 투자 보조 웹앱이다. MFRT의 월별 섹터 Rank Table과 신호를 MSS 좌측 사이드바 메뉴 항목으로 통합하여, 투자자가 종목 선정 직전에 섹터 맥락을 한 화면에서 확인할 수 있도록 한다.

```text
현재 MSS 사이드바                     MFRT 통합 후
─────────────────────                 ─────────────────────
대시보드                               대시보드
관찰종목                               관찰종목
섹터          ← 종목 레벨 섹터 신호    섹터
...
TPPS 종목검색                          TPPS 종목검색
                                  [신규] 섹터 랭킹  ← MFRT
설정 (manager only)                    설정 (manager only)
```

**연동 방식**: MSS가 MFRT SQLite DB를 직접 읽어 Streamlit 컴포넌트로 렌더링한다. MFRT HTML 파일 의존성 없음. DB 경로는 환경변수 `MFRT_DB_PATH`로 주입한다.

**파일 변경 범위**:

| 파일 | 변경 유형 | 내용 |
|---|---|---|
| `Money-Sprout-Sniffer/webapp/mss_app.py` | 수정 | 사이드바 메뉴에 `rank_table` 항목 추가 |
| `Money-Sprout-Sniffer/webapp/tabs/tab_rank_table.py` | 신규 | MFRT 데이터 렌더링 탭 |
| `Money-Sprout-Sniffer/.env` | 수정 | `MFRT_DB_PATH` 환경변수 추가 |

---

### 22.2 사이드바 메뉴 추가 (`mss_app.py`)

현재 `_render_app()` 내 데스크톱 네비게이션 코드에 `rank_table` 항목을 추가한다.

```python
# webapp/mss_app.py  (_render_app 내부 — 기존 코드에서 변경 부분만 표시)

desktop_nav_options = [
    "dashboard", "watchlist", "sector",
    "sweepbot", "moltbot", "tpps",
    "rank_table",        # ← 신규 추가
]
desktop_nav_labels = {
    "dashboard":   "대시보드",
...
```python
# _render_app() 하단 view 분기 — 기존 elif 체인 끝에 삽입

elif view == "rank_table":
    from webapp.tabs import tab_rank_table
    _render_view_safely("섹터 랭킹", tab_rank_table.render, sb)
```
...
모바일 뷰는 현재 탭 수 제한이 있어 `rank_table`을 모바일 탭에서 제외한다. 필요 시 Phase 2에서 `tab_rank_table_m.py`를 별도 작성하여 추가한다.

---

### 22.3 탭 컴포넌트 (`tab_rank_table.py`)

```python
# webapp/tabs/tab_rank_table.py
"""MoneyFlow 섹터 랭킹 탭 — MFRT SQLite DB에서 직접 로드."""
...
_MFRT_DB = os.environ.get("MFRT_DB_PATH", "moneyflow.db")
...
def _conn() -> sqlite3.Connection:
    return sqlite3.connect(_MFRT_DB)
...
def _available_months() -> list[str]:
    with _conn() as c:
        df = pd.read_sql(
            "SELECT DISTINCT trade_month FROM sector_rank_snapshot ORDER BY trade_month DESC",
            c,
        )
    return df["trade_month"].tolist()


@st.cache_data(ttl=3600, show_spinner=False)
def _load_snapshot(up_to_month: str) -> pd.DataFrame:
    with _conn() as c:
        return pd.read_sql(
            "SELECT * FROM sector_rank_snapshot WHERE trade_month <= ? ORDER BY trade_month, rank_no",
            c, params=(up_to_month,),
        )


@st.cache_data(ttl=3600, show_spinner=False)
def _load_signals(trade_month: str) -> pd.DataFrame:
    with _conn() as c:
        return pd.read_sql(
            "SELECT * FROM sector_rotation_signal WHERE trade_month = ?",
            c, params=(trade_month,),
        )


@st.cache_data(ttl=3600, show_spinner=False)
def _load_scores(trade_month: str) -> pd.DataFrame:
    with _conn() as c:
        return pd.read_sql(
            "SELECT * FROM sector_leadership_score WHERE trade_month = ?",
            c, params=(trade_month,),
...
def render(sb) -> None:
    """MSS 탭 계약: render(sb). sb (Supabase client)는 이 탭에서 사용하지 않는다."""
    st.subheader("섹터 랭킹 — MoneyFlow Rank Table")

    try:
        months = _available_months()
    except Exception as exc:
        st.error(f"MFRT DB 연결 실패: {exc}")
        st.caption(f"DB 경로 확인: MFRT_DB_PATH={_MFRT_DB}")
        return

    if not months:
        st.info("분석 데이터가 없습니다. `run_sector_monthly_pipeline` 을 먼저 실행하세요.")
        return

    col1, col2 = st.columns([2, 5])
    with col1:
        selected = st.selectbox("기준월", months, key="mfrt_month")
    with col2:
        months_back = st.slider("표시 기간 (개월)", 3, 12, 6, key="mfrt_months_back")

    snap  = _load_snapshot(selected)
    sigs  = _load_signals(selected)
    score = _load_scores(selected)

    t1, t2, t3 = st.tabs(["로테이션 매트릭스", "신호 요약", "점수 상세"])

    with t1:
        _render_matrix(snap, sigs, selected, months_back)
    with t2:
        _render_signal_summary(sigs, score)
    with t3:
        _render_score_detail(score, sigs)
...
def _render_matrix(
    ...
    """행=순위, 열=월, 현재월 셀=신호 색상."""
    months = sorted(snap["trade_month"].unique())
    display = months[-(months_back + 1):]
    pivot = (
        snap[snap["trade_month"].isin(display)]
        .pivot(index="rank_no", columns="trade_month", values="sector_name")
    )
    # 현재월 rank_change 조회 (snapshot에 저장된 값)
    chg_map = (
        snap[snap["trade_month"] == current]
        .set_index("sector_name")["rank_change"]
        .to_dict()
    )
    sig_map = sigs.set_index("sector_name")["signal"].to_dict()

    rows = []
    for rank in range(1, pivot.index.max() + 1 if len(pivot) else 22):
        row: dict = {"순위": rank}
        for m in display:
            name = pivot.loc[rank, m] if rank in pivot.index and m in pivot.columns else ""
            if m == current:
                ...
                row[f"{m} ★"] = f"{name} {arrow}".strip() if name else ""
            else:
                row[m] = name
        rows.append(row)

    df_matrix = pd.DataFrame(rows).set_index("순위")

    # 현재월 열만 배경색 적용 (pandas Styler)
    cur_col = f"{current} ★"

    def _color_cur(val: str) -> str:
        name = val.split(" ")[0] if val else ""
        sig  = sig_map.get(name, "")
        bg   = _SIGNAL_COLORS.get(sig, "#FFFFFF")
        return f"background-color: {bg}; color: {'#fff' if sig in ('LEADER','PEAK_WARNING','ROTATION_OUT') else '#000'}"

    styled = df_matrix.style
    if cur_col in df_matrix.columns:
        styled = styled.applymap(_color_cur, subset=[cur_col])
    st.dataframe(styled, use_container_width=True, height=700)


def _render_signal_summary(sigs: pd.DataFrame, score: pd.DataFrame) -> None:
    """신호 우선순위 → 점수 순 정렬. 색상 배경 행."""
    if sigs.empty:
    ...
    if not score.empty:
        df = df.merge(
            score[["sector_name", "total_score", "score_reason"]],
            on="sector_name", how="left",
        )
    df["_ord"] = df["signal"].map(_SIGNAL_ORDER).fillna(99)
    df = df.sort_values(["_ord", "total_score"], ascending=[True, False]).drop(columns=["_ord"])

    display_cols = {
        "sector_name": "업종명", "rank_no": "순위", "prev_rank_no": "전월순위",
        "signal": "신호", "total_score": "점수", "score_reason": "코멘트",
    }
    ...
    st.dataframe(
        show.style.apply(_row_color, axis=1),
        use_container_width=True,
        height=600,
    )


def _render_score_detail(score: pd.DataFrame, sigs: pd.DataFrame) -> None:
    """점수 구성요소 상세. 신호 색상 적용."""
    if score.empty:
    ...
    df = score.copy()
    if not sigs.empty:
        df = df.merge(sigs[["sector_name", "signal"]], on="sector_name", how="left")
    df = df.sort_values("total_score", ascending=False)

    keep = [
        "sector_name", "signal", "total_score", "rank_score",
        "momentum_score", "persistence_score", "expansion_score",
        "volume_score", "risk_penalty", "top10_in_3m", "top10_in_6m",
        "consecutive_top10", "score_reason",
    ]
    show = df[[c for c in keep if c in df.columns]].rename(columns={
        "sector_name": "업종명", "signal": "신호", "total_score": "합계",
        "rank_score": "순위", "momentum_score": "모멘텀",
        "persistence_score": "지속성", "expansion_score": "확산성",
        "volume_score": "거래대금", "risk_penalty": "리스크",
        "top10_in_3m": "3M", "top10_in_6m": "6M",
        "consecutive_top10": "연속", "score_reason": "코멘트",
    })
...
### 22.4 환경변수 및 DB 경로 설정

`Money-Sprout-Sniffer/.env`에 경로를 추가한다:

```bash
# Money-Sprout-Sniffer/.env  (기존 항목 유지, 아래 추가)

# ── MFRT 연동 ──────────────────────────────────────────────
# MoneyFlow Rank Table SQLite DB 경로
# MFRT 파이프라인이 생성하는 파일과 동일한 경로여야 한다.
MFRT_DB_PATH=/home/user/projects/wt/MFRT/moneyflow.db
```

`tab_rank_table.py`에서 `MFRT_DB_PATH` 미설정 시 `moneyflow.db` (현재 디렉터리)를 fallback으로 사용하므로, 경로가 맞지 않으면 `render()` 함수 상단에서 `st.error()`로 즉시 표시된다.

---

### 22.5 모바일 뷰 처리 방침

현재 모바일 뷰는 `tab_dashboard`, `tab_watchlist_m`, `tab_sector_m`, `tab_sweepbot_m` 4개 탭으로 고정되어 있어 섹터 랭킹을 추가하면 탭이 5개로 늘어난다. Streamlit `st.tabs()`는 5개 이상에서 좌우 스크롤이 발생하여 UX가 저하된다. 따라서:

- **Phase 1**: 모바일에서는 섹터 랭킹 탭을 표시하지 않는다 (현재 `mss_app.py`의 `if mobile:` 분기에 포함하지 않음).
- **Phase 2**: 모바일용 `tab_rank_table_m.py`를 별도 작성. 로테이션 매트릭스 대신 신호 카드 목록 위주 단순 레이아웃으로 구현.

---

### 22.6 투자 워크플로 통합 시나리오

```text
[MSS 일간 스캔 흐름 (PC 뷰)]

1. 섹터 랭킹 탭 열기
   → 현재월 신호 요약 확인 (NEW_LEADER / LEADER 섹터 파악)
   → 로테이션 매트릭스로 섹터 흐름 확인 (상승 중인가, 정점인가)

2. 섹터 탭으로 이동 (기존 MSS 기능)
   → 해당 섹터 내 급등 종목 신호(종목 레벨) 확인

3. 관찰종목 탭에서 후보 추가
   → MFRT 신호 + MSS 종목 신호 교차 검증

4. 매수 진입 전 §19 체크리스트 확인
   □ 지수 상태 / □ NEW_LEADER·LEADER / □ 거래대금 / □ 눌림 재돌파
```

MSS `섹터` 탭은 개별 종목의 섹터 분류 기반 신호(일봉 스캔)를 보여준다. MFRT `섹터 랭킹` 탭은 업종 지수 기준 월간 흐름을 보여준다. 두 탭은 다른 시간 프레임(일봉 vs 월봉)에서 동일한 투자 의사결정을 지원하므로 상호 보완적이다.
